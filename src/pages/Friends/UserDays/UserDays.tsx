...
export function Day({day, goal}: {day: TDay, goal: TGoal}){
    ...
    return (
            <div className={styles['single-day']} key={day.date.getTime()} >
            {
                day.progresses.length> 0? day.progresses.map(progress =>{
                    let {date} = progress;
...
export default function UserDays({ days, goals }: {days: TMyGoal[], goals: TGoal[]}){
    ...
    return (
        ...
        {goals.length > 0?  goals.map(info =>{
            ...
            const {history} = myGoal;
            const progresses = groupProgressesByDay(history);
            let dayProgress = sumDoubleDayProgress(myGoal)
